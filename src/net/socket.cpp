...
namespace Tales
{
	namespace Net
	{
		...
		bool Socket::connect(const String & host)
		{
			...
			addrSrv.sin_addr.S_un.S_addr = inet_addr(addr.str());
			addrSrv.sin_family = AF_INET;
			addrSrv.sin_port = htons(port);

			if (addrSrv.sin_addr.S_un.S_addr == INADDR_NONE) // enable DNS resolve
			{
				HOSTENT* hostent = gethostbyname(addr.str());
				addrSrv.sin_addr.S_un.S_addr = inet_addr(inet_ntoa(*(struct in_addr*)hostent->h_addr_list[0]));
			}
